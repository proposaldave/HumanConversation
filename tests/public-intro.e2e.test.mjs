...
test("the public root opens directly on the Twitter, Slack, and Human Conversation story", async () => {
  ...
  assert.deepEqual(state, {
    ...
    brandTagline: "The intelligence that brings us together.",
...
test("the promoted public root stays safe at desktop and phone sizes", async () => {
  for (const [width, height] of [
    ...
    assert.equal(layout.sectionCount, 14, `${width}x${height} opens the complete public relational-shift and previous-page story`);
    assert.equal(layout.relationalPanelCount, 3, `${width}x${height} keeps every public post-crux relational panel`);
    assert.equal(layout.imageBackedCount, 13, `${width}x${height} keeps every photographic or illustrated public section image-backed`);

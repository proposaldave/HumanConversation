...
test("the promoted public root stays safe at desktop and phone sizes", async () => {
  for (const [width, height] of [
    ...
    assert.equal(layout.sectionCount, 13, `${width}x${height} opens the complete public relational-shift and previous-page story`);
    assert.equal(layout.relationalPanelCount, 2, `${width}x${height} keeps both public post-crux relational panels`);
    assert.equal(layout.imageBackedCount, 12, `${width}x${height} keeps every photographic or illustrated public section image-backed`);
